...
#pragma warning restore 618
        ...
        private void ThrowCannotMove()
        {
            ...
            ThrowOutOfOrderKeyException(_currentKey);
        }

        // TODO docs on handling OOO
        /// <summary>
        /// Returns true if a move is valid or false if the source order changed
        /// since this cursor construction or last <see cref="MoveTo"/> move.
        /// </summary>
        ...
        public bool TryMove(long stride, bool allowPartial, out long moveCount)
        {
            ...
            {
                // MN must compare to stored version, while MT
...
//         }
        ...
        [Obsolete("Replace in MoveTo with new sync logic")]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void EnsureOrder()
        {
            if (_orderVersion != _container.OrderVersion)
            {
                ThrowOutOfOrderKeyException(_currentKey);
            }
        }

        // TODO SpreadsThrowHelper
        private void ThrowOutOfOrderKeyException(TKey key)
        {
            throw new OutOfOrderKeyException<TKey>(key);
        }
