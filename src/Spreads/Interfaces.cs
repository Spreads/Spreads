...
namespace Spreads
{
    ...
    public interface ICursor<TKey, TValue> : IEnumerator<KeyValuePair<TKey, TValue>>
    {
        ...
        /// <summary>
        /// Move the cursor to the position according to the Lookup direction. An observed value at key must exist. Use <see cref="TryGet"/> to get a calculated value for continuous series.
        /// </summary>
        /// <returns>Returns true if the cursor moved. When false is returned the cursor stays at the same position where it was before calling this method.</returns>
        bool MoveTo(TKey key, Lookup direction);
        ...
        /// and if not then .Source.GetCursor().MoveTo(key, LE).
        /// </summary>
    ...
    public interface ISeries<TKey, TValue> : IDataSource, IAsyncEnumerable<KeyValuePair<TKey, TValue>>, IDisposable
    {
        ...
        /// <summary>
        /// First key/value pair in the series. Throws <see cref="InvalidOperationException"/> if <see cref="IRowCount.IsEmpty"/> is true, so check it before accessing this property.
        /// </summary>
        KeyValuePair<TKey, TValue> First { get; }

        /// <summary>
        /// Last key/value pair in the series. Throws <see cref="InvalidOperationException"/> if <see cref="IRowCount.IsEmpty"/> is true, so check it before accessing this property.
        /// </summary>
        ...
        /// <summary>
        /// A throwing equivalent of <see cref="ICursor{TKey,TValue}.TryGet"/> and a series counterpart of <see cref="ICursor{TKey,TValue}.TryGet"/>.
        /// </summary>
        ...
        /// The <paramref name="key"/> parameter is overwritten if the found key is different from the search key.
        /// </summary>
        /// <remarks>
        /// For unsorted series (<see cref="KeySorting.NotSorted"/>) this method throws <see cref="InvalidOperationException"/>
        /// if <paramref name="direction"/> is not <see cref="Lookup.EQ"/>.
        /// TryFind works only with observed keys and is a series counterpart of <see cref="ICursor{TKey,TValue}.MoveTo"/>.
        /// </remarks>
        /// <exception cref="InvalidOperationException">
        /// The <paramref name="direction">direction</paramref> is not <see cref="Lookup.EQ"/> for unsorted series (<see cref="KeySorting.NotSorted"/>).
        /// </exception>
        bool TryFind(ref TKey key, Lookup direction, out TValue value);
    ...
    public interface IAppendSeries<TKey, TValue> : ISeries<TKey, TValue>
    {
        ...
        ISeries<TKey, TValue> ToReadOnly();
    }
