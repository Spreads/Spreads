...
namespace Spreads
{
    public struct ZipOp<TKey, TValue, TOp, TCursorLeft, TCursorRight>
        : ICursor<TKey, TValue, ZipOp<TKey, TValue, TOp, TCursorLeft, TCursorRight>>
        where TCursorLeft : ICursor<TKey, TValue, TCursorLeft>
...
#pragma warning restore HAA0601 // Value type to reference type conversion causing boxing allocation
    ...
    /// <summary>
    /// An <see cref="ISeries{TKey,TValue,TCursor}"/> that applies an arithmetic operation
    /// <see cref="IOp{TValue}"/> to each value of its input series.
    /// </summary>
    public struct Op2<TKey, TValue, TOp, TCursor> :
        ICursor<TKey, TValue, Op2<TKey, TValue, TOp, TCursor>>
        where TCursor : ICursor<TKey, (TValue, TValue), TCursor>
        where TOp : struct, IOp<TValue>
    {
