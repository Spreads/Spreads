...
namespace Spreads
{
	...
	internal class SpreadsContractResolver : DefaultContractResolver
	{
		...
		protected override JsonConverter ResolveContractConverter(Type ty)
		{
			if (ty.IsGenericType &&
					ty.GetGenericTypeDefinition() == typeof(SortedMap<,>))
			{
				// TODO custom converter that will use static method and write binary value
	...
	{ // TODO internal
		...
		internal class SerializerInstance : ISerializer
		{
			...
			public byte[] Serialize(object obj0)
			{
				throw new NotImplementedException();
			}

			public object Deserialize(byte[] obj0, Type obj1)
			{
				throw new NotImplementedException();
			}
		}
		...
		static Serializer()
		{
			...
			// are called before the serializer is used, so libblosc is loaded.
		...
		/// 
		/// </summary>
		public static byte[] CompressMap<K, V>(SortedMap<K, V> src,
		   ...
		   int? typeSize = null, CompressionMethod? method = null)
		{
			...
			FixedBufferTransformer<Tuple<IntPtr, int>> passThrough = (source, length, self) =>
			{
				unsafe
				{
			...
			Task<Tuple<IntPtr, int>> cKeysTask;
			if (isRegular)
			{
				// use sign bit as a flag, regular vs irregular by definition do not expect a third option
				size = -size;
				cKeysTask = Task.FromResult(SerializeTransform(keys[0], passThrough));
			}
			else
			{
				cKeysTask = Task.Run(() => CompressArray<K, Tuple<IntPtr, int>>(keys, passThrough, 0, size, level, shuffle, typeSize, method, true));
			}
			...
			{
				var keysPtr = cKeysTask.Result.Item1;
				var keysLen = cKeysTask.Result.Item2;
				...
				Marshal.Copy(keysPtr, ret, 12, keysLen);
				Buffer.BlockCopy(valBytes, 0, ret, 12 + keysLen, valLen);
#if PRERELEASE
				if (!wroteSize)
				{ // debug stuff
					Console.WriteLine("Compressed size: " + retLen);
					wroteSize = true;
				}
#endif
			...
			return CompressArray<V, TResult>(values, buildReturnArray, 0, size, level, shuffle, typeSize, method, true);
			//var cValues =
			//// values offset
			//bw.Write(cKeys.Length + 4 + 4 + 4);
			//bw.Write(cKeys);
			//bw.Write(cValues);
			//bw.Close();
			//var arr = ms.ToArray();
		...
		public static SortedMap<K, V> DecompressMapPtr<K, V>(IntPtr srcPtr)
		{
			...
			var keys = DecompressArray<K>(srcPtr, keyStart, true);
			var values = DecompressArray<V>(srcPtr, valueStart, true);
		...
		internal static bool MapKeysAreCertainlyEqual<K, V>(IntPtr srcPtrA, IntPtr srcPtrB)
		{
			...
			return memcmp(srcPtrA + 12, srcPtrB + 12, sizeA) == 0;
		}
		...
		/// <returns></returns>
		public static byte[] CompressArray<T>(T[] src, int start = 0, int length = 0,
			int? level = null, bool? shuffle = null,
			int? typeSize = null, CompressionMethod? method = null, bool diff = true)
		{
			...
			int? typeSize = null, CompressionMethod? method = null, bool diff = true)
		{
			...
			)
			{
				unsafe
				{
					var numThreads = 1;//Environment.ProcessorCount);
					checked
					{
			...
			if (ty == typeof(string))
			{
				shuffle1 = shuffle ?? false;
				//method1 = method ?? CompressionMethod.zlib;
			}
			return CompressBytes<TResult>(bytes, bufferTransform, level1, shuffle1, typeSize1, method1);
		...
		/// 
		/// </summary>
		public static unsafe T[] DecompressArray<T>(byte[] src, int start = 0, bool diff = true)
		{
			if (src == null) throw new ArgumentNullException("src");
			if (src.Length == 0) throw new ArgumentException("src is empty");
		...
		/// 
		/// </summary>
		internal static unsafe T[] DecompressArray<T>(IntPtr srcPtr, int start = 0, bool diff = true)
		{
				...
				)
			{
				...
				if (ty == typeof(DateTime))
				{
					UInt64[] newSrc = DecompressArray<UInt64>(srcPtr, start, false);
					DateTime[] dts = new DateTime[newSrc.Length];
					var previous = 0UL;
					for (int i = 0; i < dts.Length; i++)
					{
						var current = newSrc[i] + previous;
						dts[i] = new DateTime((long)(current & ~(3UL << 62)), (DateTimeKind)(current >> 62));
				...
				{
					decimal[] newSrc = DecompressArray<decimal>(srcPtr, start, false);
					...
					{
						var current = newSrc[i] + previous;
						doubles[i] = Convert.ToDouble(current);
						if (diff) previous = current;
					}
					return (T[])(object)doubles;
				}
				...
				{
					long[] newSrc = DecompressArray<long>(srcPtr, start, false);
					var previous = 0L;
					for (int i = 0; i < newSrc.Length; i++)
					{
						var current = newSrc[i] + previous;
				...
				else
				{
					...
					unsafe
					{
						...
						NativeMethods.Library.blosc_cbuffer_sizes(
						...
						using (var gp = new GenericArrayPinner<T>(dest))
						{
							...
							var decompSize = NativeMethods.Library.blosc_decompress_ctx(
			...
			int typeSize = 1, CompressionMethod method = CompressionMethod.lz4)
		{
			...
			var dest = new byte[maxLength];
			var destSize = new UIntPtr((uint)maxLength);
			var numThreads = 1;//System.Environment.ProcessorCount);
			unsafe
			{
		...
		internal static byte[] SerializeImpl<T>(T value)
		...
		internal static T Deserialize<T>(IntPtr srcPtr, int srcSize)
		{
			...
			else
			{
					...
					ty.GetGenericTypeDefinition() == typeof(SortedMap<,>))
				{
					...
					return (T)genericMi.Invoke(null, new object[] { srcPtr });
				}
				else if (ty.IsArray)
				{
					...
					{
						return (T)(object)DecompressBytes(srcPtr, srcSize);
					}
					else
					{
						var hasSaved = genericMethods.TryGetValue(ty, out genericMi);
				...
				{
					return (T)(object)DeserializeImpl(srcPtr, srcSize, String.Empty);
				}
				else
				{
					return (T)DeserializeImpl(srcPtr, srcSize, typeof(T));
				}
			}
			return r;
		}
