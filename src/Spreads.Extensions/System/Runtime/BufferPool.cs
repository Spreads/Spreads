// Spreads: Adapted from .NET, changed byte[] to generic T[]
		...
		internal class PooledBufferManager : InternalBufferManager<T> {
			...
			internal abstract class BufferPool {
				...
				internal static BufferPool CreatePool(int bufferSizeInBytes, int limit) {
					...
					// bufferSizeInBytes < 85000, the SynchronizedPool. However if bufferSizeInBytes < 85000
					// and (bufferSizeInBytes + array-overhead) > 85000, this would still use 
					// the SynchronizedPool even though it is allocated on the LOH.

					// VB: http://blogs.msdn.com/b/dotnet/archive/2011/10/04/large-object-heap-improvements-in-net-4-5.aspx
					// In 32-bit architrctures CLR’s execution engine attempts to place these arrays > 1000 doubles on the LOH 
					// because of the performance benefit of accessing aligned doubles. However there are no benefits of 
					// applying the same heuristics on 64-bit architectures because doubles are already aligned on an 8-byte
					// boundary. As such we have disabled this heuristics for 64-bit architectures in .,NET 4.5
					if (Bootstrapper.ABI.Equals(ABI.Windows_X86) && typeof (T) == typeof (double) && bufferSizeInBytes > 8000)
					{
						return new LargeBufferPool(bufferSizeInBytes, limit);
					}
					else if (bufferSizeInBytes < 85000-24) { // VB: added -24
