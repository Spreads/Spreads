...
namespace Spreads.Serialization {
    ...
    [Obsolete("This is old version to resque old data")]
    public static class SerializerOld { // TODO internal

        internal class SerializerInstance : ISerializer {
            byte[] ISerializer.Serialize<T>(T value) {
                return SerializerOld.Serialize(value);
            }

            T ISerializer.Deserialize<T>(byte[] bytes) {
                return SerializerOld.Deserialize<T>(bytes);
            }
