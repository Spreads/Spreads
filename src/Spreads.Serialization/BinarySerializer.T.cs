...
{
    /// <summary>
    /// Serializes a generic object T to a <see cref="Span{T}"/> of <see cref="byte"/>, prefixed with version and length.
    /// </summary>
    public interface IBinarySerializer<T>
    {
        ...
        /// <returns></returns>
        int SizeOf(in T value, BufferWriter payload);
        ...
        /// positive length without writing anything to the provided <see cref="BufferWriter"/> payload.
        /// </summary>
        ...
        int Deserialize(Span<byte> source, out T value);

        int DeserializeInto(Span<byte> source, ref T value);
    }
