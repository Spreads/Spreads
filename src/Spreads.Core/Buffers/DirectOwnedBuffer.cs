...
namespace Spreads.Buffers
{
    ...
    {
        internal OwnedBuffer<byte> _lockers;
        ...
        /// The buffer could be null. DirectOwnedBuffer owns the lockers buffer.
        /// </summary>
        internal DirectOwnedBuffer(OwnedBuffer<byte> lockersBuffer, T[] array, int arrayOffset, int length, IntPtr pointer = new IntPtr()) : base(array, arrayOffset, length, pointer)
        {
            if (lockersBuffer != null && lockersBuffer.Length != 64)
            {
                throw new ArgumentException("Lockers buffer must have length of 64 bytes.");
        ...
        {
            // locker, version, next version, size, process ref count + padding to fill cache line
            Init(OwnedBuffer<byte>.Create(new ArraySegment<byte>(new byte[8 * 8])));
        }
        ...
        {
            Init(OwnedBuffer<byte>.Create(new ArraySegment<byte>(new byte[8 * 4])));
        }
        ...
        private void Init(OwnedBuffer<byte> lockers)
        {
            if (lockers != null)
            {
                _lockers = lockers;
