...
// Design goals/notes
// * Blittable types use aligned native memory. We align to page size even when using allocators (not VirtualAlloc/mmap directly).
// * Pool has limited number of buffers, which are for bursts and reducing freeing of frequently recycled buffers. Allocating
//   new buffers should be OK.
// * We should proactively monitor pool state instead of reacting to low memory
//   - if there were allocations over the last X seconds then increase target count by 2
//   - if minimum remaining was above target count then reduce target count by 1
// * MemoryManager objects should be pooled aggressively, especially for blittable types
// * All reference types should use shared object[] pool
// * TLS only for bytes, with 64kb buffer
...
{
    // TODO (spreads)
    // Keep track of lowest number of buffers in a bucket per second
    // Use TLS only for blittables (or bytes) and limit size

    internal unsafe class NonGeneric
    {
        [ThreadStatic] private static byte* tlsBuffer;
        private const int TlsBufferSize = 64 * 1024;
    }
    ...
    /// </remarks>
    internal sealed unsafe partial  class TlsOverPerCoreLockedStacksArrayPool<T> : ArrayPool<T>
    {
        
        // https://github.com/dotnet/runtime/blob/e2dafdfcb3500e1305e26f90d89d66fa35280426/src/libraries/Common/src/System/Net/WebSockets/ManagedWebSocket.cs
        ...
        private const int
            MaxPerCorePerArraySizeStacks = 64; // selected to avoid needing to worry about processor groups
        ...
        [ThreadStatic] private static T[]?[]? t_tlsBucketsx;
        ...
        private sealed class PerCoreLockedStacks
        {
            ...
            public void TryPush(T[] array)
            {
                ...
                int index = ProcessorIdCache.GetCurrentProcessorId() % stacks.Length;
                for (int i = 0; i < stacks.Length; i++)
                {
