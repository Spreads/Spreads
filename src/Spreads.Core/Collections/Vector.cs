...
namespace Spreads.Experimental {
    ...
    // of the Vector is automatically increased as required by reallocating the
    // internal array.
    // 
    ...
    public class Vector<T> : IList<T>, System.Collections.IList, IReadOnlyList<T>, IDisposable {
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                if (value != _items.Length) {
                    if (value > 0) {
                        var items = ArrayPool<T>.Shared.Rent(value);
        ...
        // Read-only property describing how many elements are in the Vector.
        public int Count
        {
            get
            {
        ...
        // Contains returns true if the specified element is in the Vector.
        ...
        // Copies this Vector into array, which must be of a 
        ...
        public int FindLastIndex(int startIndex, int count, Predicate<T> match) {
            ...
            if (_size == 0) {
                // Special case for 0 length Vector
                if (startIndex != -1) {
                    throw new ArgumentOutOfRangeException(nameof(startIndex), startIndex, "SR.ArgumentOutOfRange_Index");
                }
        ...
        // to the end of the list by setting index to the Vector's size.
        //
        public void InsertRange(int index, IEnumerable<T> collection) {
            ...
            if (c != null) {    // if collection is ICollection<T>
                ...
                if (count > 0) {
                    ...
                    // If we're inserting a Vector into itself, we want to be able to deal with that.
                    if (this == c) {
                        ...
                        T[] itemsToInsert = ArrayPool<T>.Shared.Rent(count);
                        c.CopyTo(itemsToInsert, 0);
                        Array.Copy(itemsToInsert, 0, _items, index, count);
                        // clear array in background
                        Task.Run(() => ArrayPool<T>.Shared.Return(itemsToInsert, true));
                    }
                    _size += count;
                }
            } else {
        ...
        // ToArray returns a new Object array containing the contents of the Vector.
        // This requires copying the Vector, which is an O(n) operation.
        public T[] ToArray() {
        ...
        public struct Enumerator : IEnumerator<T>, System.Collections.IEnumerator {
            ...
            internal Enumerator(Vector<T> list) {
                this.list = list;
                index = 0;
