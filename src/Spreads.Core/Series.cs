...
namespace Spreads
{
    ...
    public partial class Series<TKey, TValue> : BaseContainer<TKey>, ISeries<TKey, TValue, SCursor<TKey, TValue>>
    {
        ...
        internal Series(Mutability mutability = Mutability.Mutable, 
            KeySorting keySorting = KeySorting.Strong,
            uint capacity = 0,
            KeyComparer<TKey> comparer = default,
            uint minCircularCapacity = 0)
        {
            ...
            // TODO review/test
            var bufferSize = Math.Min(Settings.LARGE_BUFFER_LIMIT / Unsafe.SizeOf<TKey>(), (int) capacity);
            BlockSizePow2 = Math.Max((byte)4, checked((byte) (32 - IntUtil.NumberOfLeadingZeros(bufferSize - 1))));

            _comparer = comparer;

            if (minCircularCapacity != 0)
            {
                ThrowHelper.ThrowNotImplementedException();
            }
        }

        internal Series(int capacity) : this(capacity, default)
        {

        }

        internal Series(int capacity, KeyComparer<TKey> comparer) 
            : this(Mutability.Mutable, KeySorting.Strong, (uint)capacity, comparer)
        {
        }
