...
namespace Spreads
{
    ...
    //    public AsyncCursor(BaseSeries<TK, TV> source, ref TCursor cursor) {
    //        this._source = source;
    //        _innerCursor = cursor;
    ...
    //            _token = cancellationToken;
    //            _cancelledTcs = new TaskCompletionSource<Task<bool>>();
    //            _registration = _token.Register(SetCancelledResult);
    //        }
    ...
    //    {
    //        _cancelledTcs.SetResult(TaskEx.CancelledBoolTask);
    //    }

    //    // TODO benchmark it as a local function
    //    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    //    private Task<bool> MoveNextContinuation(Task<long> t) {
    //        // while this is not null, no one would be able to set a new one
    //        var original = Volatile.Read(ref _source.UpdateTcs);
    //        if (original != null) {
    //            // one of many cursors will succeed
    //            Interlocked.CompareExchange(ref _source.UpdateTcs, null, original);
