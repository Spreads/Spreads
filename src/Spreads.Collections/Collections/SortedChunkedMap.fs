...
//  member this.Clear() : unit = this.RemoveMany(this.First.Present.Key, Lookup.GE) |> ignore

//  override this.Comparer with get() = comparer

//  [<MethodImplAttribute(MethodImplOptions.AggressiveInlining);RewriteAIL>]
//  member inline private this.PrevBucketIsSet (prevBucket':SortedMap<'K,'V> byref) : bool =
...
//          let mutable prevBucket' = Unchecked.defaultof<_>
//          if this.PrevBucketIsSet(&prevBucket') && comparer.Compare(key, prevHash) >= 0 && comparer.Compare(key, prevBucket'.Last.Present.Key) <= 0 then
//            prevBucket'.[key]
//          else
//            let mutable kvp = Unchecked.defaultof<_>
//            let ok = outerMap.TryFindAt(key, Lookup.LE, &kvp)
//            if ok then
//              prevHash <- kvp.Key
...
//          let mutable prevBucket' = Unchecked.defaultof<_>
//          let bucketIsSet = this.PrevBucketIsSet(&prevBucket')
//          if bucketIsSet && comparer.Compare(key, prevHash) >= 0 && comparer.Compare(key, prevBucket'.Last.Present.Key) <= 0 then
...
//              (kvp.Value.size < chunkUpperLimit || comparer.Compare(key, kvp.Value.Last.Present.Key) <= 0) then
...
//              #else
//              outerMap.Set(key, newSm) // outerMap.Version is incremented here, set non-empty bucket only
//              #endif
//              prevHash <- key
...
//      let res, pair =
//        let mutable prevBucket' = Unchecked.defaultof<_>
//        if c <> 0 || (not <| this.PrevBucketIsSet(&prevBucket')) then // not in the prev bucket, switch bucket to newHash
...
//          if this.PrevBucketIsSet(&prevBucket') then
...
//        else comparer.Compare(key, this.LastUnchecked.Key)
//      if c > 0 then
//        this.AddUnchecked(key, value)
//      else
//        let exn = OutOfOrderKeyException(this.LastUnchecked.Key, key, "New key is smaller or equal to the largest existing key")
//        raise (exn)
//      added <- true
//    finally
//      if added then Interlocked.Increment(&this.version) |> ignore elif entered then Interlocked.Decrement(&this.nextVersion) |> ignore
...
//      let c = 
//        if outerMap.IsEmpty then -1
//        else comparer.Compare(key, this.FirstUnchecked.Key)
//      if c < 0 then 
//        this.AddUnchecked(key, value)
//      else 
//        let exn = OutOfOrderKeyException(this.LastUnchecked.Key, key, "New key is larger or equal to the smallest existing key")
//        raise (exn)
//      added <- true
//    finally
//      if added then Interlocked.Increment(&this.version) |> ignore elif entered then Interlocked.Decrement(&this.nextVersion) |> ignore
...
//    let c = comparer.Compare(hash, prevHash)
//    let mutable prevBucket' = Unchecked.defaultof<_>
//    let prevBucketIsSet = this.PrevBucketIsSet(&prevBucket')
...
//      let factory = (fun (c:KeyComparer<'K>) -> new ChunksContainer<'K,'V>(c, false) :> IMutableSeries<'K,'V,SortedMap<'K,'V>>)
//      new SortedChunkedMap<_,_>(factory, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), comparer, None, None)
...
//      new SortedChunkedMap<_,_>(factory, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), KeyComparer<'K>.Create(comparer), None, None)
...
//          }
//      new SortedChunkedMap<_,_>(factory, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), comparer, Some(hasher), None)
...
//      new SortedChunkedMap<_,_>(factory, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), comparer, None, Some(chunkMaxSize))

//    internal new(outerFactory:Func<IComparer<'K>,IMutableChunksSeries<'K,'V, SortedMap<'K,'V>>>) = 
//      let comparer = KeyComparer<'K>.Default
//      new SortedChunkedMap<_,_>(outerFactory.Invoke, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), comparer, None, None)
...
//      new SortedChunkedMap<_,_>(outerFactory.Invoke, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), KeyComparer<'K>.Create(comparer), None, None)
...
//            member x.Hash(k) = hasher.Invoke k
//          }
//      new SortedChunkedMap<_,_>(outerFactory.Invoke, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), comparer, Some(hasher), None)
  
//    internal new(outerFactory:Func<IComparer<'K>,IMutableChunksSeries<'K,'V,SortedMap<'K,'V>>>,chunkMaxSize:int) = 
//      let comparer = KeyComparer<'K>.Default
//      new SortedChunkedMap<_,_>(outerFactory.Invoke, (fun (capacity, comparer) -> new SortedMap<'K,'V>(capacity, comparer, IsSynchronized = false)), comparer, None, Some(chunkMaxSize))
