...
module internal KeyHelper = 
  ...
  let isRegular<'K when 'K:comparison> (sortedArray:'K[]) (size:int) (dc:ISpreadsComparer<'K>) = 
    let lastOffset = size - 1
    Debug.Assert(sortedArray.Length >= size)
    dc.Diff(sortedArray.[lastOffset], sortedArray.[0])  = lastOffset

  let willRemainRegular<'K  when 'K:comparison> (start:'K) (size:int) (dc:ISpreadsComparer<'K>) (newValue:'K) : bool = 
    if size = 0 then true
    else 
      //      0 || 4    1,2,3
      dc.Diff(newValue, start) = size || dc.Diff(newValue, start) = -1

  let toArray<'K  when 'K:comparison> (start:'K) (size:int) (length:int) (dc:ISpreadsComparer<'K>) : 'K[] =
