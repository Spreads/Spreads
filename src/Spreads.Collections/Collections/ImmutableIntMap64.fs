...
type internal IntMap64Tree<'T> =
    | Nil
    | Tip of int64 * 'T
    | Bin of int64 * int64 * IntMap64Tree<'T> * IntMap64Tree<'T> // Prefix * Mask * Map * Map
    ...
    member x.FoldBackWithKey f z =
        ...
        match x with
        | Bin(_, m, l, r) when m < 0L -> go (go z l) r  // put negative numbers before.
        | Bin(_, m, l, r) -> go (go z r) l
        | _ -> go z x
    
    member x.ToList() = x.FoldBackWithKey (fun k x xs -> (k, x) :: xs) []

    interface IEnumerable<int64 * 'T> with
...
module internal IntMap64Tree =

    let inline private maskW i m = int64 (i &&& (~~~ (m - 1UL) ^^^ m)) // Nat -> Nat -> Prefix
    ...
    let inline private branchMask p1 p2 = int64 (highestBitMask (uint64 p1 ^^^ uint64 p2))
    ...
    let rec find k m =
        let notFound() = raise (System.Collections.Generic.KeyNotFoundException(sprintf "IntMap64Tree.find: key %d is not an element of the map" k))
        match m with
    ...
    let tryFindLT k t =
        ...
        match t with
        | Bin(_, m, l, r) when m < 0L -> if k >= 0L then go r l else go Nil r
        | _ -> go Nil t
    ...
    let tryFindGT k t =
        ...
        match t with
        | Bin(_, m, l, r) when m < 0L -> if k >= 0L then go Nil l else go l r
        | _ -> go Nil t
    ...
    ///O(log n). The expression (alter f k map) alters the value x at k, or absence thereof. alter can be used to insert, delete, or update a value in an IntMap64Tree. Credit: Haskell.org
    let rec alter f k t =
        match t with
    ...
    let updateMinWithKey f t =
        ...
        match t with
        | Bin(p, m, l, r) when m < 0L -> bin p m l (go f r)
        | _ -> go f t
    ...
    let updateMaxWithKey f t =
        ...
        match t with
        | Bin(p, m, l, r) when m < 0L -> bin p m (go f l) r
        | _ -> go f t
    ...
    let maxViewWithKey t =
        ...
        match t with
        | Nil -> None
        | Bin(p, m, l, r) when m < 0L -> let (result, l) = go l in Some(result, bin p m l r)
        | _ -> Some(go t)
    ...
    let minViewWithKey t =
        ...
        match t with
        | Nil -> None
        | Bin(p, m, l, r) when m < 0L -> let (result, r) = go r in Some(result, bin p m l r)
        | _ -> Some(go t)
    ...
    let split k t =
        let rec go k t =
            ...
            | Nil -> (Nil, Nil)
        match t with
        | Bin(_, m, l, r) when  m < 0L ->
            if k >= 0L // handle negative numbers.
    ...
    let fold f z =
        ...
        fun t ->
            match t with
            | Bin(_, m, l, r) when m < 0L -> go (go z r) l  // put negative numbers before.
    ...
    let inline foldBackWithKey f z = fun (t: _ IntMap64Tree) -> t.FoldBackWithKey f z
    ...
    let inline toList (m: _ IntMap64Tree) = m.ToList()
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    type MapPointer<'V> = 
            ...
            mutable stack: IntMap64Tree<'V> list;  
            /// true when Move was called
            mutable positioned : bool }

    let private mkPointerStack (k:int64) (m:IntMap64Tree<'V>) : IntMap64Tree<'V> list = 
        let rec mkPointerStackAux k (m:IntMap64Tree<'V>) (stack:IntMap64Tree<'V> list) : IntMap64Tree<'V> list =
            //let notFound() = raise (System.Collections.Generic.KeyNotFoundException(sprintf "IntMap64Tree.find: key %d is not an element of the map" k))
            match m with
            | Bin(p, m, l, r) when nomatch k p m -> [] //notFound()
    ...
    let private mkPointerStackMin (m:IntMap64Tree<'V>) : IntMap64Tree<'V> list = 
        let rec mkPointerStackAuxMin (m:IntMap64Tree<'V>) (stack:IntMap64Tree<'V> list) : IntMap64Tree<'V> list=
    ...
    let private mkPointerStackMax (m:IntMap64Tree<'V>) : IntMap64Tree<'V> list = 
        let rec mkPointerStackAuxMax (m:IntMap64Tree<'V>) (stack:IntMap64Tree<'V> list) : IntMap64Tree<'V> list=
            //let notFound() = raise (System.Collections.Generic.KeyNotFoundException())
            match m with
    ...
    let mkPointer (m:IntMap64Tree<'V>) : MapPointer<_> = 
    ...
    let current (pointer:MapPointer<'V>) : int64*'V =
    ...
    let moveNext (pointer:MapPointer<'V>) : bool =
        ...
        let rec ascendR = function //(stack:IntMap64Tree<'V> list) : (IntMap64Tree<'V> list) =
            //match stack with
...
// type IntMap64Tree is the underlying data structure, internal
// module IntMap64Tree contains algos to modify IntMap64Tree
// public type ImmutableIntMap64 in the only public API, design follows F#.Core.Map
...
namespace Spreads.Collections
    ...
    type ImmutableIntMap64<[<EqualityConditionalOn;ComparisonConditionalOn>]'T>
        ...
        static member Create(elements : IEnumerable<KeyValuePair<int64, 'T>>) : ImmutableIntMap64<'T> = 
           new ImmutableIntMap64<'T>(IntMap64Tree.ofSeq (elements |> Seq.map (fun x -> x.Key,x.Value) ))
    
        static member Create() : ImmutableIntMap64<'T> = empty

        new(elements : IEnumerable<KeyValuePair<int64, 'T>>) = new ImmutableIntMap64<'T>(IntMap64Tree.ofSeq (elements |> Seq.map (fun x -> x.Key,x.Value) ))
        ...
        member this.IsEmpty
                with get() = IntMap64Tree.isEmpty tree

        member this.First
            with get() = 
                let res = IntMap64Tree.tryFindMin tree
        ...
        member this.Last
            with get() = 
                let res = IntMap64Tree.tryFindMax tree
                if res.IsNone then raise (InvalidOperationException("Could not get the last element of an empty map"))
                KeyValuePair(fst res.Value, snd res.Value)

        member this.Item
            with get key =
                let tr = IntMap64Tree.tryFind key tree
                if tr.IsNone then raise (KeyNotFoundException())
                tr.Value

        member this.TryFind(key,direction:Lookup, [<Out>]result: byref<KeyValuePair<int64, 'T>>) : bool =
            result <- Unchecked.defaultof<KeyValuePair<int64, 'T>>
            ...
            | Lookup.EQ -> 
                ...
                if tr.IsNone then 
                    ...
                    result <- KeyValuePair<int64, 'T>(key, tr.Value)
                    true
            | Lookup.LT -> 
                let tr = IntMap64Tree.tryFindLT key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int64, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.LE -> 
                let tr = IntMap64Tree.tryFindLE key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int64, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.GT -> 
                let tr = IntMap64Tree.tryFindGT key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int64, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.GE ->
                let tr = IntMap64Tree.tryFindGE key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int64, 'T>(fst tr.Value, snd tr.Value)
        ...
        interface IImmutableSortedMap<int64, 'T> with
...
//            member this.Count with get(): int = int this.Size
            ...
            member this.Add(key, value):IImmutableSortedMap<int64,'T> =
                this.Add(key, value) :> IImmutableSortedMap<int64,'T>

            member this.AddFirst(key, value):IImmutableSortedMap<int64,'T> =
                this.AddFirst(key, value) :> IImmutableSortedMap<int64,'T>

            member this.AddLast(key, value):IImmutableSortedMap<int64,'T> =
                this.AddLast(key, value) :> IImmutableSortedMap<int64,'T>

            member this.Remove(key):IImmutableSortedMap<int64,'T> =
                this.Remove(key) :> IImmutableSortedMap<int64,'T>

            member this.RemoveLast([<Out>] value: byref<KeyValuePair<int64, 'T>>):IImmutableSortedMap<int64,'T> =
                let m,v = this.RemoveLast()
                value <- v
                m :> IImmutableSortedMap<int64,'T>

            member this.RemoveFirst([<Out>] value: byref<KeyValuePair<int64, 'T>>):IImmutableSortedMap<int64,'T> =
                let m,v = this.RemoveFirst()
                value <- v
                m :> IImmutableSortedMap<int64,'T>

            member this.RemoveMany(key,direction:Lookup):IImmutableSortedMap<int64,'T>=
                this.RemoveMany(key, direction) :> IImmutableSortedMap<int64,'T>
