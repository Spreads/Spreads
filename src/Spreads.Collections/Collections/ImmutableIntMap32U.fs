...
type internal IntMap32UTree<'T> =
    | Nil
    | Tip of uint32 * 'T
    | Bin of uint32 * uint32 * IntMap32UTree<'T> * IntMap32UTree<'T> // Prefix * Mask * Map * Map
    ...
    member x.FoldBackWithKey f z =
        ...
        match x with
        | Bin(_, m, l, r) when m < 0u -> go (go z l) r  // put negative numbers before.
        | Bin(_, m, l, r) -> go (go z r) l
        | _ -> go z x
    
    member x.ToList() = x.FoldBackWithKey (fun k x xs -> (k, x) :: xs) []

    interface IEnumerable<uint32 * 'T> with
...
module internal IntMap32UTree =

    let inline private maskW i m = uint32 (i &&& (~~~ (m - 1u) ^^^ m)) // Nat -> Nat -> Prefix
    let inline private mask i m = maskW (uint32 i) (uint32 m) // Key -> Mask -> Prefix
    ...
    let inline private zero i m = (uint32 i) &&& (uint32 m) = 0u 
    let inline private shorter m1 m2 = (uint32 m1) > (uint32 m2) // Mask -> Mask -> Bool // with Big-endian a larger mask will be closer to the roor
    ...
    let inline private highestBitMask x0 =
        ...
        let x4 = x3 ||| (x3 >>> 8)
        let x5 = x4 ||| (x4 >>> 16)
        let x6 = x5 ||| (x5 >>> 32)   // for 32 bit platforms
        x6 ^^^ (x6 >>> 1)

    let inline private branchMask p1 p2 = uint32 (highestBitMask (uint32 p1 ^^^ uint32 p2))
    ...
    let rec find k m =
        let notFound() = raise (System.Collections.Generic.KeyNotFoundException(sprintf "IntMap32UTree.find: key %d is not an element of the map" k))
        match m with
    ...
    let tryFindLT k t =
        ...
        match t with
        | Bin(_, m, l, r) when m < 0u -> if k >= 0u then go r l else go Nil r
        | _ -> go Nil t
    ...
    let tryFindGT k t =
        ...
        match t with
        | Bin(_, m, l, r) when m < 0u -> if k >= 0u then go Nil l else go l r
        | _ -> go Nil t
    ...
    ///O(log n). The expression (alter f k map) alters the value x at k, or absence thereof. alter can be used to insert, delete, or update a value in an IntMap32UTree. Credit: Haskell.org
    let rec alter f k t =
        match t with
    ...
    let updateMinWithKey f t =
        ...
        match t with
        | Bin(p, m, l, r) when m < 0u -> bin p m l (go f r)
        | _ -> go f t
    ...
    let updateMaxWithKey f t =
        ...
        match t with
        | Bin(p, m, l, r) when m < 0u -> bin p m (go f l) r
        | _ -> go f t
    ...
    let maxViewWithKey t =
        ...
        match t with
        | Nil -> None
        | Bin(p, m, l, r) when m < 0u -> let (result, l) = go l in Some(result, bin p m l r)
        | _ -> Some(go t)
    ...
    let minViewWithKey t =
        ...
        match t with
        | Nil -> None
        | Bin(p, m, l, r) when m < 0u -> let (result, r) = go r in Some(result, bin p m l r)
        | _ -> Some(go t)
    ...
    let split k t =
        let rec go k t =
            ...
            | Nil -> (Nil, Nil)
        match t with
        | Bin(_, m, l, r) when  m < 0u ->
            if k >= 0u // handle negative numbers.
    ...
    let fold f z =
        ...
        fun t ->
            match t with
            | Bin(_, m, l, r) when m < 0u -> go (go z r) l  // put negative numbers before.
    ...
    let inline foldBackWithKey f z = fun (t: _ IntMap32UTree) -> t.FoldBackWithKey f z
    ...
    let inline toList (m: _ IntMap32UTree) = m.ToList()
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    type MapPointer<'V> = 
            ...
            mutable stack: IntMap32UTree<'V> list;  
            /// true when Move was called
            mutable positioned : bool }

    let private mkPointerStack (k:uint32) (m:IntMap32UTree<'V>) : IntMap32UTree<'V> list = 
        let rec mkPointerStackAux k (m:IntMap32UTree<'V>) (stack:IntMap32UTree<'V> list) : IntMap32UTree<'V> list =
            //let notFound() = raise (System.Collections.Generic.KeyNotFoundException(sprintf "IntMap32UTree.find: key %d is not an element of the map" k))
            match m with
            | Bin(p, m, l, r) when nomatch k p m -> [] //notFound()
    ...
    let private mkPointerStackMin (m:IntMap32UTree<'V>) : IntMap32UTree<'V> list = 
        let rec mkPointerStackAuxMin (m:IntMap32UTree<'V>) (stack:IntMap32UTree<'V> list) : IntMap32UTree<'V> list=
    ...
    let private mkPointerStackMax (m:IntMap32UTree<'V>) : IntMap32UTree<'V> list = 
        let rec mkPointerStackAuxMax (m:IntMap32UTree<'V>) (stack:IntMap32UTree<'V> list) : IntMap32UTree<'V> list=
            //let notFound() = raise (System.Collections.Generic.KeyNotFoundException())
            match m with
    ...
    let mkPointer (m:IntMap32UTree<'V>) : MapPointer<_> = 
    ...
    let current (pointer:MapPointer<'V>) : uint32*'V =
    ...
    let moveNext (pointer:MapPointer<'V>) : bool =
        ...
        let rec ascendR = function //(stack:IntMap32UTree<'V> list) : (IntMap32UTree<'V> list) =
            //match stack with
...
// type IntMap32UTree is the underlying data structure, internal
// module IntMap32UTree contains algos to modify IntMap32UTree
// public type ImmutableIntMap32U in the only public API, design follows F#.Core.Map
...
namespace Spreads.Collections
    ...
    [<Sealed>]
    [<CompiledName("ImmutableIntMap32U`1")>]
    type ImmutableIntMap32U<[<EqualityConditionalOn;ComparisonConditionalOn>]'T>
        internal(tree: IntMap32UTree<'T>)=
        let syncRoot = new Object()
        static let empty = ImmutableIntMap32U<'T>(IntMap32UTree.empty)

        static member Empty : ImmutableIntMap32U<'T> = empty

        static member Create(elements : IEnumerable<KeyValuePair<uint32, 'T>>) : ImmutableIntMap32U<'T> = 
           new ImmutableIntMap32U<'T>(IntMap32UTree.ofSeq (elements |> Seq.map (fun x -> x.Key,x.Value) ))
    
        static member Create() : ImmutableIntMap32U<'T> = empty

        new(elements : IEnumerable<KeyValuePair<uint32, 'T>>) = new ImmutableIntMap32U<'T>(IntMap32UTree.ofSeq (elements |> Seq.map (fun x -> x.Key,x.Value) ))
        ...
        member this.IsEmpty
                with get() = IntMap32UTree.isEmpty tree

        member this.First
            with get() = 
                let res = IntMap32UTree.tryFindMin tree
        ...
        member this.Last
            with get() = 
                let res = IntMap32UTree.tryFindMax tree
                if res.IsNone then raise (InvalidOperationException("Could not get the last element of an empty map"))
                KeyValuePair(fst res.Value, snd res.Value)

        member this.Item
            with get key =
                let tr = IntMap32UTree.tryFind key tree
                if tr.IsNone then raise (KeyNotFoundException())
                tr.Value

        member this.TryFind(key,direction:Lookup, [<Out>]result: byref<KeyValuePair<uint32, 'T>>) : bool =
            result <- Unchecked.defaultof<KeyValuePair<uint32, 'T>>
            ...
            | Lookup.EQ -> 
                ...
                if tr.IsNone then 
                    ...
                    result <- KeyValuePair<uint32, 'T>(key, tr.Value)
                    true
            | Lookup.LT -> 
                let tr = IntMap32UTree.tryFindLT key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<uint32, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.LE -> 
                let tr = IntMap32UTree.tryFindLE key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<uint32, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.GT -> 
                let tr = IntMap32UTree.tryFindGT key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<uint32, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.GE ->
                let tr = IntMap32UTree.tryFindGE key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<uint32, 'T>(fst tr.Value, snd tr.Value)
        ...
        member this.GetCursor() = new MapCursor<uint32,'T>(this) :> ICursor<uint32,'T>
        ...
        member this.Add(k, v):ImmutableIntMap32U<'T> = ImmutableIntMap32U(IntMap32UTree.insert k v tree)

        member this.AddFirst(k, v):ImmutableIntMap32U<'T> = 
        ...
        member this.AddLast(k, v):ImmutableIntMap32U<'T> = 
        ...
        member this.Remove(k):ImmutableIntMap32U<'T> = ImmutableIntMap32U(IntMap32UTree.delete k tree)

        member this.RemoveLast([<Out>]value: byref<KeyValuePair<uint32, 'T>>):ImmutableIntMap32U<'T>=
            if this.IsEmpty then raise (InvalidOperationException("Could not get the last element of an empty map"))
            let v, newTree = IntMap32UTree.deleteFindMax tree
            value <- KeyValuePair(fst v, snd v)
            ImmutableIntMap32U(newTree)

        member this.RemoveFirst([<Out>]value: byref<KeyValuePair<uint32, 'T>>):ImmutableIntMap32U<'T>=
            if this.IsEmpty then raise (InvalidOperationException("Could not get the first element of an empty map"))
            let v, newTree = IntMap32UTree.deleteFindMin tree
            value <- KeyValuePair(fst v, snd v)
            ImmutableIntMap32U(newTree)

        member this.RemoveMany(k:uint32, direction:Lookup):ImmutableIntMap32U<'T>=
            let newTree = 
                match direction with
                | Lookup.LT ->
                    snd (IntMap32UTree.split (k - 1u) tree)
                | Lookup.LE ->
                    snd (IntMap32UTree.split (k) tree) 
                | Lookup.GT ->
                    fst (IntMap32UTree.split (k + 1u) tree)
                | Lookup.GE ->
                    fst (IntMap32UTree.split (k) tree)
                | Lookup.EQ ->
                    IntMap32UTree.delete k tree
                | _ -> failwith "unexpected wrong direction"
            ImmutableIntMap32U(newTree)

        interface IEnumerable<KeyValuePair<uint32, 'T>> with
            member m.GetEnumerator() = 
                (tree.ToList().ToArray() 
                |> Array.map (fun (k,v) -> KeyValuePair(k,v)) :> (KeyValuePair<uint32, 'T>) seq).GetEnumerator()
            member m.GetEnumerator() =
                (m :> _ seq).GetEnumerator() :> IEnumerator

   

        interface IImmutableSeries<uint32, 'T> with
            member this.Subscribe(observer) = raise (NotImplementedException())
            member this.Comparer with get() = KeyComparer.GetDefault<uint32>()
            ...
            member this.TryFind(k, direction:Lookup, [<Out>] res: byref<KeyValuePair<uint32, 'T>>) = 
            ...
            member this.TryGetFirst([<Out>] res: byref<KeyValuePair<uint32, 'T>>) = 
                try
                    res <- this.First
                    true
                with
            ...
            member this.TryGetLast([<Out>] res: byref<KeyValuePair<uint32, 'T>>) = 
                try
                    res <- this.Last
                    true
                with
...
//            member this.Count with get() = int(this.Size)
            ...
            member this.Add(key, value):IImmutableSeries<uint32,'T> =
                this.Add(key, value) :> IImmutableSeries<uint32,'T>

            member this.AddFirst(key, value):IImmutableSeries<uint32,'T> =
                this.AddFirst(key, value) :> IImmutableSeries<uint32,'T>

            member this.AddLast(key, value):IImmutableSeries<uint32,'T> =
                this.AddLast(key, value) :> IImmutableSeries<uint32,'T>

            member this.Remove(key):IImmutableSeries<uint32,'T> =
                this.Remove(key) :> IImmutableSeries<uint32,'T>

            member this.RemoveLast([<Out>] value: byref<KeyValuePair<uint32, 'T>>):IImmutableSeries<uint32,'T> =
                let m,v = this.RemoveLast()
                value <- v
                m :> IImmutableSeries<uint32,'T>

            member this.RemoveFirst([<Out>] value: byref<KeyValuePair<uint32, 'T>>):IImmutableSeries<uint32,'T> =
                let m,v = this.RemoveFirst()
                value <- v
                m :> IImmutableSeries<uint32,'T>

            member this.RemoveMany(key,direction:Lookup):IImmutableSeries<uint32,'T>=
                this.RemoveMany(key, direction) :> IImmutableSeries<uint32,'T>
