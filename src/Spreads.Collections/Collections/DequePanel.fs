...
type SortedDequePanel<'K,'V>
  ...
  member this.Item
    with get key =
      ...
      try
        ...
        exitLockIf syncRoot entered
    and set k v =
      if this.isReadOnly then invalidOp "SortedDequePanel is read-only"
      this.sd.Set(KVP(k,v))
      this.version <- this.version + 1
  ...
  override this.GetCursor() = new SortedDequePanelCursor<_,_>(this) :> ICursor<_,_>

  // foreach optimization
  member this.GetEnumerator() = new SortedDequePanelCursor<'K,'V>(this)
  //#endregion
...
//        this.AddLast(i.Key, i.Value)
  ...
  new() = SortedDequePanel(None, None, None)
  new(comparer:IComparer<'K>) = SortedDequePanel(None, None, Some(comparer))
  new(dictionary:IDictionary<'K,'V>) = SortedDequePanel(Some(dictionary), Some(dictionary.Count), None)
  new(capacity:int) = SortedDequePanel(None, Some(capacity), None)
  new(dictionary:IDictionary<'K,'V>,comparer:IComparer<'K>) = SortedDequePanel(Some(dictionary), Some(dictionary.Count), Some(comparer))
  new(capacity:int,comparer:IComparer<'K>) = SortedDequePanel(None, Some(capacity), Some(comparer))

  static member internal OfKeysAndValues(size:int, keys:'K[],values:'V[]) =
    let sdm = new SortedDequePanel<'K,'V>()
...
and
  SortedDequePanelCursor<'K,'V> =
    struct
      val public source : SortedDequePanel<'K,'V>
      val mutable index : int
      val mutable current : KVP<'K,'V>
      val mutable cursorVersion : int
      val mutable isBatch : bool

      new(source:SortedDequePanel<'K,'V>) = 
        { source = source; 
          index = -1;
    ...
    member this.Clone() = 
      let mutable clone = new SortedDequePanelCursor<'K,'V>(this.source)
