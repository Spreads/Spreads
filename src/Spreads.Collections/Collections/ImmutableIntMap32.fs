...
type internal IntMap32Tree<'T> =
    | Nil
    | Tip of int32 * 'T
    | Bin of int32 * int32 * IntMap32Tree<'T> * IntMap32Tree<'T> // Prefix * Mask * Map * Map
    ...
    interface IEnumerable<int32 * 'T> with
...
module internal IntMap32Tree =

    let inline private maskW i m = int32 (i &&& (~~~ (m - 1u) ^^^ m)) // Nat -> Nat -> Prefix
    ...
    let inline private branchMask p1 p2 = int32 (highestBitMask (uint32 p1 ^^^ uint32 p2))
    ...
    let rec find k m =
        let notFound() = raise (System.Collections.Generic.KeyNotFoundException(sprintf "IntMap32Tree.find: key %d is not an element of the map" k))
        match m with
    ...
    ///O(log n). The expression (alter f k map) alters the value x at k, or absence thereof. alter can be used to insert, delete, or update a value in an IntMap32Tree. Credit: Haskell.org
    let rec alter f k t =
        match t with
    ...
    let inline foldBackWithKey f z = fun (t: _ IntMap32Tree) -> t.FoldBackWithKey f z
    ...
    let inline toList (m: _ IntMap32Tree) = m.ToList()
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    type MapPointer<'V> = 
            ...
            mutable stack: IntMap32Tree<'V> list;  
            /// true when Move was called
            mutable positioned : bool }

    let private mkPointerStack (k:int32) (m:IntMap32Tree<'V>) : IntMap32Tree<'V> list = 
        let rec mkPointerStackAux k (m:IntMap32Tree<'V>) (stack:IntMap32Tree<'V> list) : IntMap32Tree<'V> list =
            //let notFound() = raise (System.Collections.Generic.KeyNotFoundException(sprintf "IntMap32Tree.find: key %d is not an element of the map" k))
            match m with
            | Bin(p, m, l, r) when nomatch k p m -> [] //notFound()
    ...
    let private mkPointerStackMin (m:IntMap32Tree<'V>) : IntMap32Tree<'V> list = 
        let rec mkPointerStackAuxMin (m:IntMap32Tree<'V>) (stack:IntMap32Tree<'V> list) : IntMap32Tree<'V> list=
    ...
    let private mkPointerStackMax (m:IntMap32Tree<'V>) : IntMap32Tree<'V> list = 
        let rec mkPointerStackAuxMax (m:IntMap32Tree<'V>) (stack:IntMap32Tree<'V> list) : IntMap32Tree<'V> list=
            //let notFound() = raise (System.Collections.Generic.KeyNotFoundException())
            match m with
    ...
    let mkPointer (m:IntMap32Tree<'V>) : MapPointer<_> = 
    ...
    let current (pointer:MapPointer<'V>) : int32*'V =
    ...
    let moveNext (pointer:MapPointer<'V>) : bool =
        ...
        let rec ascendR = function //(stack:IntMap32Tree<'V> list) : (IntMap32Tree<'V> list) =
            //match stack with
...
// type IntMap32Tree is the underlying data structure, internal
// module IntMap32Tree contains algos to modify IntMap32Tree
// public type ImmutableIntMap32 in the only public API, design follows F#.Core.Map
...
namespace Spreads.Collections
    ...
    [<Sealed>]
    [<CompiledName("ImmutableIntMap32`1")>]
    [<Serializable>]
    type ImmutableIntMap32<[<EqualityConditionalOn;ComparisonConditionalOn>]'T>
        internal(tree: IntMap32Tree<'T>)=
        ...
        let mutable serializedData : KeyValuePair<int32,'T> array = null
#endif

        static let empty = ImmutableIntMap32<'T>(IntMap32Tree.empty)
        ...
        member internal __.OnDeserialized(context: System.Runtime.Serialization.StreamingContext) =
            ignore(context)
            tree <- serializedData |> Array.map (fun (KeyValue(k,v)) -> (k,v)) |> IntMap32Tree.ofArray 
            serializedData <- null
#endif

        static member Empty : ImmutableIntMap32<'T> = empty

        static member Create(elements : IEnumerable<KeyValuePair<int32, 'T>>) : ImmutableIntMap32<'T> = 
           new ImmutableIntMap32<'T>(IntMap32Tree.ofSeq (elements |> Seq.map (fun x -> x.Key,x.Value) ))
    
        static member Create() : ImmutableIntMap32<'T> = empty

        new(elements : IEnumerable<KeyValuePair<int32, 'T>>) = new ImmutableIntMap32<'T>(IntMap32Tree.ofSeq (elements |> Seq.map (fun x -> x.Key,x.Value) ))
        ...
        member this.IsEmpty
                with get() = IntMap32Tree.isEmpty tree

        member this.First
            with get() = 
                let res = IntMap32Tree.tryFindMin tree
        ...
        member this.Last
            with get() = 
                let res = IntMap32Tree.tryFindMax tree
                if res.IsNone then raise (InvalidOperationException("Could not get the last element of an empty map"))
                KeyValuePair(fst res.Value, snd res.Value)

        member this.Item
            with get key =
                let tr = IntMap32Tree.tryFind key tree
                if tr.IsNone then raise (KeyNotFoundException())
                tr.Value

        member this.TryFind(key,direction:Lookup, [<Out>]result: byref<KeyValuePair<int32, 'T>>) : bool =
            result <- Unchecked.defaultof<KeyValuePair<int32, 'T>>
            ...
            | Lookup.EQ -> 
                ...
                if tr.IsNone then 
                    ...
                    result <- KeyValuePair<int32, 'T>(key, tr.Value)
                    true
            | Lookup.LT -> 
                let tr = IntMap32Tree.tryFindLT key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int32, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.LE -> 
                let tr = IntMap32Tree.tryFindLE key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int32, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.GT -> 
                let tr = IntMap32Tree.tryFindGT key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int32, 'T>(fst tr.Value, snd tr.Value)
                    true
            | Lookup.GE ->
                let tr = IntMap32Tree.tryFindGE key tree
                if tr.IsNone then 
                    false
                else
                    result <- KeyValuePair<int32, 'T>(fst tr.Value, snd tr.Value)
        ...
        member this.GetPointer() = new BasePointer<int32,'T>(this) :> IPointer<int32,'T>
        ...
        member this.Add(k, v):ImmutableIntMap32<'T> = ImmutableIntMap32(IntMap32Tree.insert k v tree)

        member this.AddFirst(k, v):ImmutableIntMap32<'T> = 
        ...
        member this.AddLast(k, v):ImmutableIntMap32<'T> = 
        ...
        member this.Remove(k):ImmutableIntMap32<'T> = ImmutableIntMap32(IntMap32Tree.delete k tree)

        member this.RemoveLast([<Out>]value: byref<KeyValuePair<int32, 'T>>):ImmutableIntMap32<'T>=
            if this.IsEmpty then raise (InvalidOperationException("Could not get the last element of an empty map"))
            let v, newTree = IntMap32Tree.deleteFindMax tree
            value <- KeyValuePair(fst v, snd v)
            ImmutableIntMap32(newTree)

        member this.RemoveFirst([<Out>]value: byref<KeyValuePair<int32, 'T>>):ImmutableIntMap32<'T>=
            if this.IsEmpty then raise (InvalidOperationException("Could not get the first element of an empty map"))
            let v, newTree = IntMap32Tree.deleteFindMin tree
            value <- KeyValuePair(fst v, snd v)
            ImmutableIntMap32(newTree)

        member this.RemoveMany(k:int32, direction:Lookup):ImmutableIntMap32<'T>=
            let newTree = 
                match direction with
                | Lookup.LT ->
                    snd (IntMap32Tree.split (k - 1) tree)
                | Lookup.LE ->
                    snd (IntMap32Tree.split (k) tree) 
                | Lookup.GT ->
                    fst (IntMap32Tree.split (k + 1) tree)
                | Lookup.GE ->
                    fst (IntMap32Tree.split (k) tree)
                | Lookup.EQ ->
                    IntMap32Tree.delete k tree
                | _ -> failwith "unexpected wrong direction"
            ImmutableIntMap32(newTree)

        interface IEnumerable<KeyValuePair<int32, 'T>> with
            member m.GetEnumerator() = 
                (tree.ToList().ToArray() 
                |> Array.map (fun (k,v) -> KeyValuePair(k,v)) :> (KeyValuePair<int32, 'T>) seq).GetEnumerator()
            member m.GetEnumerator() =
                (m :> _ seq).GetEnumerator() :> IEnumerator

    

        interface IImmutableSortedMap<int32, 'T> with
            ...
            member this.Keys with get() = IntMap32Tree.keys tree :> IEnumerable<int>
            member this.Values with get() = IntMap32Tree.values tree :> IEnumerable<'T>

            member this.TryFind(k, direction:Lookup, [<Out>] res: byref<KeyValuePair<int32, 'T>>) = 
                res <- Unchecked.defaultof<KeyValuePair<int32, 'T>>
            ...
            member this.TryGetFirst([<Out>] res: byref<KeyValuePair<int32, 'T>>) = 
                try
                    res <- this.First
                    true
                with
            ...
            member this.TryGetLast([<Out>] res: byref<KeyValuePair<int32, 'T>>) = 
                try
                    res <- this.Last
                    true
                with
...
//            member this.Count with get() = int(this.Size)
            ...
            member this.Add(key, value):IImmutableSortedMap<int32,'T> =
                this.Add(key, value) :> IImmutableSortedMap<int32,'T>

            member this.AddFirst(key, value):IImmutableSortedMap<int32,'T> =
                this.AddFirst(key, value) :> IImmutableSortedMap<int32,'T>

            member this.AddLast(key, value):IImmutableSortedMap<int32,'T> =
                this.AddLast(key, value) :> IImmutableSortedMap<int32,'T>

            member this.Remove(key):IImmutableSortedMap<int32,'T> =
                this.Remove(key) :> IImmutableSortedMap<int32,'T>

            member this.RemoveLast([<Out>] value: byref<KeyValuePair<int32, 'T>>):IImmutableSortedMap<int32,'T> =
                let m,v = this.RemoveLast()
                value <- v
                m :> IImmutableSortedMap<int32,'T>

            member this.RemoveFirst([<Out>] value: byref<KeyValuePair<int32, 'T>>):IImmutableSortedMap<int32,'T> =
                let m,v = this.RemoveFirst()
                value <- v
                m :> IImmutableSortedMap<int32,'T>

            member this.RemoveMany(key,direction:Lookup):IImmutableSortedMap<int32,'T>=
                this.RemoveMany(key, direction) :> IImmutableSortedMap<int32,'T>
