...
namespace Spreads.Collections
    ...
    type ImmutableIntMap32<[<EqualityConditionalOn;ComparisonConditionalOn>]'T>
        ...
        interface IImmutableSeries<int32, 'T> with
          member this.Updated = falseTask
          member this.Subscribe(observer) = raise (NotImplementedException())
          member this.Comparer with get() = KeyComparer<int32>.Default
...
//            member this.Count with get() = int(this.Size)
          ...
          member this.AddFirst(key, value):IImmutableSeries<int32,'T> =
              this.AddFirst(key, value) :> IImmutableSeries<int32,'T>

          member this.AddLast(key, value):IImmutableSeries<int32,'T> =
              this.AddLast(key, value) :> IImmutableSeries<int32,'T>

          member this.Remove(key):IImmutableSeries<int32,'T> =
              this.Remove(key) :> IImmutableSeries<int32,'T>

          member this.RemoveLast([<Out>] value: byref<KeyValuePair<int32, 'T>>):IImmutableSeries<int32,'T> =
              let m,v = this.RemoveLast()
              value <- v
              m :> IImmutableSeries<int32,'T>

          member this.RemoveFirst([<Out>] value: byref<KeyValuePair<int32, 'T>>):IImmutableSeries<int32,'T> =
              let m,v = this.RemoveFirst()
              value <- v
              m :> IImmutableSeries<int32,'T>

          member this.RemoveMany(key,direction:Lookup):IImmutableSeries<int32,'T>=
              this.RemoveMany(key, direction) :> IImmutableSeries<int32,'T>
