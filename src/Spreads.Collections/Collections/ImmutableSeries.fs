...
type MapTree<'K,'V when 'K : comparison > = 
  | MapEmpty 
  | MapOne of 'K * 'V
...
module MapTree = 
  ...
  let rec add (comparer: KeyComparer<'K>) k v m = // was 'V instead of 'K, slightly measleading
      match m with 
      | MapEmpty -> MapOne(k,v)
      | MapOne(k2,_) -> 
  ...
  let rec find (comparer: KeyComparer<'K>) k m = 
      match m with 
  ...
  //[<MethodImpl(MethodImplOptions.AggressiveInlining)>]
  //let inline find (comparer: KeyComparer<'K>) k m : 'a = 
  //    let mutable found = false
  //    let mutable m = m
  //    let mutable result = Unchecked.defaultof<'a>
  //    while not found do
  //        match m with 
  //        | MapEmpty -> ThrowHelper.ThrowKeyNotFoundException("")
  //        | MapOne(k2,v2) -> 
  //            let c = comparer.Compare(k,k2) 
  //            if c = 0 then 
  //              result <- v2
  //              found <- true
  //            else ThrowHelper.ThrowKeyNotFoundException("")
  //        | MapNode(k2,v2,l,r,_) -> 
  //            let c = comparer.Compare(k,k2) 
  //            if c < 0 then
  //              m <- l
