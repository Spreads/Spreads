...
type SortedPanel<'TRowKey,'TColumnKey,'TValue> // when 'K:equality
  internal(dictionary:IDictionary<'TRowKey,ISeries<'TColumnKey,'TValue>> option, capacity:int option, comparerOpt:IComparer<'TRowKey> option) as this=
  ...
  val mutable internal rowKeys : 'TRowKey array
  [<DefaultValueAttribute>]
  val mutable internal columnKeys : 'TColumnKey array
  [<DefaultValueAttribute>]
  val mutable internal values : 'TValue array // row-major matrix of values
  ...
  let mutable comparer : IComparer<'TRowKey> = 
    if comparerOpt.IsNone || Comparer<'TRowKey>.Default.Equals(comparerOpt.Value) then
      let kc = KeyComparer.GetDefault<'TRowKey>()
      if kc = Unchecked.defaultof<_> then Comparer<'TRowKey>.Default :> IComparer<'TRowKey> 
      else kc
    else comparerOpt.Value
  [<NonSerializedAttribute>]
  let isKeyReferenceType : bool = not typeof<'TRowKey>.IsValueType
  [<NonSerializedAttribute>]
  ...
  let updateEvent = new Internals.EventV2<UpdateHandler<'TRowKey,'TValue>,KVP<'TRowKey,'TValue>>()
  ...
  do
    ...
    if dictionary.IsNone then // otherwise we will set them in dict processing part
      this.rowKeys <- OptimizationSettings.ArrayPool.TakeBuffer tempCap
    ...
    if dictionary.IsSome && dictionary.Value.Count > 0 then
      ...
      | _ ->
        ...
        dictionary.Value.Keys.CopyTo(this.rowKeys, 0)
  ...
  member private this.Clone() = new IndexedMap<'TRowKey,'TValue>(Some(this :> IDictionary<'TRowKey,'TValue>), None, Some(comparer))

  member internal this.GetKeyByIndex(index) =
    this.rowKeys.[index]
    
  member private this.GetPairByIndexUnchecked(index) = //inline
    KeyValuePair(this.rowKeys.[index], this.values.[index]) 
  ...
  member private this.Insert(index:int, k, v) = 
    ...
    if index < this.size then
      Array.Copy(this.rowKeys, index, this.rowKeys, index + 1, this.size - index);
      Array.Copy(this.values, index, this.values, index + 1, this.size - index);
    this.rowKeys.[index] <- k
    this.values.[index] <- v     
    version <- version + 1
  ...
  member this.Capacity
    ...
    and set(value) =
      ...
      try
        ...
        | c when c > 0 -> 
          let kArr : 'TRowKey array = OptimizationSettings.ArrayPool.TakeBuffer(c)
          Array.Copy(this.rowKeys, 0, kArr, 0, this.size)
          let toReturn = this.rowKeys
          this.rowKeys <- kArr
          OptimizationSettings.ArrayPool.ReturnBuffer(toReturn)

          let vArr : 'TValue array = OptimizationSettings.ArrayPool.TakeBuffer(c)
  ...
  member this.Clear() =
    version <- version + 1
    Array.Clear(this.rowKeys, 0, this.size)
  ...
  member this.Keys 
    with get() : IList<'TRowKey> =
      {new IList<'TRowKey> with
        member x.Count with get() = this.size
        member x.IsReadOnly with get() = true
        member x.Item 
          with get index : 'TRowKey = this.GetKeyByIndex(index)
        ...
        member x.CopyTo(array, arrayIndex) = 
          Array.Copy(this.rowKeys, 0, array, arrayIndex, this.size)
        member x.IndexOf(key:'TRowKey) = this.IndexOfKey(key)
        member x.Insert(index, value) = raise (NotSupportedException("Keys collection is read-only"))
        member x.Remove(key:'TRowKey) = raise (NotSupportedException("Keys collection is read-only"))
        ...
        member x.GetEnumerator() : IEnumerator<'TRowKey> = 
          let index = ref 0
          let eVersion = ref version
          let currentKey : 'TRowKey ref = ref Unchecked.defaultof<'TRowKey>
          { new IEnumerator<'TRowKey> with
            ...
            member e.MoveNext() = 
              ...
              if index.Value < this.size then
                currentKey := this.rowKeys.[!index]
                index := index.Value + 1
                true
              else
                index := this.size + 1
                currentKey := Unchecked.defaultof<'TRowKey>
                false
            member e.Reset() = 
              if eVersion.Value <> version then
                raise (InvalidOperationException("Collection changed during enumeration"))
  ...
  member this.Values 
    with get() : IList<'TValue> =
      { new IList<'TValue> with
        member x.Count with get() = this.size
        member x.IsReadOnly with get() = true
        member x.Item 
          with get index : 'TValue = this.values.[index]
        ...
        member x.IndexOf(value:'TValue) = this.IndexOfValue(value)
        member x.Insert(index, value) = raise (NotSupportedException("Values colelction is read-only"))
        member x.Remove(value:'TValue) = raise (NotSupportedException("Values colelction is read-only"))
        ...
        member x.GetEnumerator() : IEnumerator<'TValue> = 
          let index = ref 0
          let eVersion = ref version
          let currentValue : 'TValue ref = ref Unchecked.defaultof<'TValue>
          { new IEnumerator<'TValue> with
            ...
            member e.MoveNext() = 
              ...
              if index.Value < this.size then
                ...
                index := this.size + 1
                currentValue := Unchecked.defaultof<'TValue>
                false
            member e.Reset() = 
              if eVersion.Value <> version then
                raise (InvalidOperationException("Collection changed during enumeration"))
  ...
  member internal this.IndexOfKeyUnchecked(key:'TRowKey) : int =
    let entered = enterLockIf syncRoot  isSynchronized
    try
      let mutable res = 0
      let mutable found = false
      while not found do
          if comparer.Compare(key, this.rowKeys.[res]) = 0 then
              found <- true
          else res <- res + 1
      if found then res else ~~~this.rowKeys.Length // add to the end
    finally
      exitLockIf syncRoot entered

  member this.IndexOfKey(key:'TRowKey) : int =
    if isKeyReferenceType && EqualityComparer<'TRowKey>.Default.Equals(key, Unchecked.defaultof<'TRowKey>) then 
      raise (ArgumentNullException("key"))
    this.IndexOfKeyUnchecked(key)

  member this.IndexOfValue(value:'TValue) : int =
    let entered = enterLockIf syncRoot  isSynchronized
    try
      let mutable res = 0
      let mutable found = false
      let valueComparer = Comparer<'TValue>.Default;
      while not found do
  ...
  member this.First
    with get() = 
      ...
      KeyValuePair(this.rowKeys.[0], this.values.[0])

  member this.Last
    with get() =
      if this.size = 0 then raise (InvalidOperationException("Could not get the last element of an empty map"))
      KeyValuePair(this.rowKeys.[this.size - 1], this.values.[this.size - 1])
  ...
  member this.Item
      with get key =
        ...
        try
          ...
          exitLockIf syncRoot entered
      and set k v =
        if isKeyReferenceType && EqualityComparer<'TRowKey>.Default.Equals(k, Unchecked.defaultof<'TRowKey>) then 
          raise (ArgumentNullException("key"))
  ...
  member internal this.SetWithIndex(k, v) =
    ...
    try
      ...
      if this.size = 0 then
        ...
        if comparer.Compare(k, this.rowKeys.[lastIdx]) = 0 then // key = last key
  ...
  member internal this.RemoveAt(index):unit =
    ...
    try
      ...
      if index < this.size then
        Array.Copy(this.rowKeys, index + 1, this.rowKeys, index, newSize - index) // this.size
        Array.Copy(this.values, index + 1, this.values, index, newSize - index) //this.size

      this.rowKeys.[newSize] <- Unchecked.defaultof<'TRowKey>
      this.values.[newSize] <- Unchecked.defaultof<'TValue>
  ...
  member this.RemoveFirst([<Out>]result: byref<KVP<'TRowKey,'TValue>>):bool =
    let entered = enterLockIf syncRoot  isSynchronized
    try
      try
  ...
  member this.RemoveLast([<Out>]result: byref<KeyValuePair<'TRowKey, 'TValue>>):bool =
    let entered = enterLockIf syncRoot  isSynchronized
    try
      try
  ...
  member this.RemoveMany(key:'TRowKey,direction:Lookup):bool =
    let entered = enterLockIf syncRoot  isSynchronized
    try
      if this.size = 0 then false
        ...
        | Lookup.LT | Lookup.LE ->
          if pivotIndex = -1 then // pivot is not here but to the left, keep all elements
            ...
            version <- version + 1
            Array.Copy(this.rowKeys, pivotIndex + 1, this.rowKeys, 0, this.size) // move this.values to 
            Array.fill this.rowKeys this.size (this.values.Length - this.size) Unchecked.defaultof<'TRowKey>

            Array.Copy(this.values, pivotIndex + 1, this.values, 0, this.size)
            Array.fill this.values this.size (this.values.Length - this.size) Unchecked.defaultof<'TValue>
            true
          else
        ...
        | Lookup.GT | Lookup.GE ->
          if pivotIndex = -2 then // pivot is not here but to the right, keep all elements
            ...
            Array.fill this.rowKeys pivotIndex (this.values.Length - pivotIndex) Unchecked.defaultof<'TRowKey>
            Array.fill this.values pivotIndex (this.values.Length - pivotIndex) Unchecked.defaultof<'TValue>
            version <- version + 1
  ...
  member internal this.TryFindWithIndex(key:'TRowKey,direction:Lookup, [<Out>]result: byref<KeyValuePair<'TRowKey, 'TValue>>) : int = // rkok
    let entered = enterLockIf syncRoot  isSynchronized
    try
      if this.size = 0 then -4
        ...
        | Lookup.EQ ->
          let lastIdx = this.size-1
          if this.size > 0 && comparer.Compare(key, this.rowKeys.[lastIdx]) = 0 then // key = last key
        ...
        | Lookup.LT ->
          let lastIdx = this.size-1
          let lc = if this.size > 0 then comparer.Compare(key, this.rowKeys.[lastIdx]) else -2
          ...
          if lc = 0 then // key = last key or greater than the last key
            ...
            if index >= 0 then
              ...
              if index2 >= this.size then // there are no elements larger than key
                ...
                index2 - 1
        | Lookup.GT ->
          let lc = if this.size > 0 then comparer.Compare(key, this.rowKeys.[0]) else 2
  ...
  member this.TryFind(k:'TRowKey, direction:Lookup, [<Out>] res: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
    res <- Unchecked.defaultof<KeyValuePair<'TRowKey, 'TValue>>
  ...
  member this.TryGetValue(key, [<Out>]value: byref<'TValue>) : bool =
    let entered = enterLockIf syncRoot  isSynchronized
    try
      ...
      if this.size = 0 then
        ...
        let lc = comparer.Compare(key, this.rowKeys.[this.size - 1])
        if lc = 0 then // key = last key
  ...
  member this.TryGetFirst([<Out>] res: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
    try
      res <- this.First
      true
    with
  ...
  member this.TryGetLast([<Out>] res: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
    try
      res <- this.Last
      true
    with
  ...
  override this.GetCursor() = this.GetCursor(-1, version, Unchecked.defaultof<'TRowKey>, Unchecked.defaultof<'TValue>)
  ...
  member internal this.GetCursor(index:int,cursorVersion:int,currentKey:'TRowKey, currentValue:'TValue) =
    //IndexedMapCursor(this, index, cursorVersion, currentKey, currentValue) :> ICursor<'K,'V>
    let index = ref index
    let cursorVersion = ref cursorVersion
    let currentKey : 'TRowKey ref = ref currentKey
    let currentValue : 'TValue ref = ref currentValue
    let isBatch = ref false
    { 
    new BaseCursor<'TRowKey,'TValue>(this) with
      override this.IsContinuous with get() = false
      override p.CurrentBatch: IReadOnlyOrderedMap<'TRowKey,'TValue> = 
        if !index = -1 then this :> IReadOnlyOrderedMap<'TRowKey,'TValue>
      ...
      override p.MoveAt(key:'TRowKey, lookup:Lookup) = 
        let entered = enterLockIf syncRoot  isSynchronized
        try
  ...
  interface IEnumerable<KeyValuePair<'TRowKey,'TValue>> with
    member this.GetEnumerator() : IEnumerator<KeyValuePair<'TRowKey,'TValue>> = 
      this.GetCursor() :> IEnumerator<KeyValuePair<'TRowKey,'TValue>>
  ...
  interface IDictionary<'TRowKey,'TValue> with
    member this.Count = this.Count
    member this.IsReadOnly with get() = this.IsReadOnly
    member this.Item
      with get key = this.Item(key)
      and set key value = this.[key] <- value
    member this.Keys with get() = this.Keys :> ICollection<'TRowKey>
    member this.Values with get() = this.Values :> ICollection<'TValue>
    member this.Clear() = this.Clear()
    member this.ContainsKey(key) = this.ContainsKey(key)
    member this.Contains(kvp:KeyValuePair<'TRowKey,'TValue>) = this.ContainsKey(kvp.Key)
    member this.CopyTo(array, arrayIndex) =
      if array = null then raise (ArgumentNullException("array"))
      if arrayIndex < 0 || arrayIndex > array.Length then raise (ArgumentOutOfRangeException("arrayIndex"))
      if array.Length - arrayIndex < this.Count then raise (ArgumentException("ArrayPlusOffTooSmall"))
    ...
    member this.Add(kvp:KeyValuePair<'TRowKey,'TValue>) = this.Add(kvp.Key, kvp.Value)
    member this.Remove(key) = this.Remove(key)
    member this.Remove(kvp:KeyValuePair<'TRowKey,'TValue>) = this.Remove(kvp.Key)
    member this.TryGetValue(key, [<Out>]value: byref<'TValue>) : bool =
      let index = this.IndexOfKey(key)
      if index >= 0 then
        value <- this.values.[index]
  ...
  interface IReadOnlyOrderedMap<'TRowKey,'TValue> with
    member this.Comparer with get() = this.Comparer
    member this.GetEnumerator() = this.GetCursor() :> IAsyncEnumerator<KVP<'TRowKey, 'TValue>>
    member this.GetCursor() = this.GetCursor()
    member this.IsEmpty = this.size = 0
    member this.IsIndexed with get() = false
    member this.First with get() = this.First
    member this.Last with get() = this.Last
    member this.TryFind(k:'TRowKey, direction:Lookup, [<Out>] result: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
    ...
    member this.TryGetLast([<Out>] res: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
      ...
      | _ -> 
        res <- Unchecked.defaultof<KeyValuePair<'TRowKey, 'TValue>>
        false
    member this.TryGetValue(k, [<Out>] value:byref<'TValue>) = this.TryGetValue(k, &value)
    member this.Item with get k = this.Item(k)
    member this.Keys with get() = this.Keys :> IEnumerable<'TRowKey>
    member this.Values with get() = this.values :> IEnumerable<'TValue>
    member this.SyncRoot with get() = syncRoot
    

  interface IOrderedMap<'TRowKey,'TValue> with
    member this.Count with get() = int64(this.size)
    member this.Item with get k = this.Item(k) and set (k:'TRowKey) (v:'TValue) = this.[k] <- v
    ...
    member this.RemoveFirst([<Out>] result: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
      this.RemoveFirst(&result)
    member this.RemoveLast([<Out>] result: byref<KeyValuePair<'TRowKey, 'TValue>>) = 
      this.RemoveLast(&result)
    member this.RemoveMany(key:'TRowKey,direction:Lookup) = 
      this.RemoveMany(key, direction)

    // TODO move to type memeber, cheack if IROOM is SM and copy arrays in one go
    member this.Append(appendMap:IReadOnlyOrderedMap<'TRowKey,'TValue>, option:AppendOption) =
      let hasEqOverlap (old:IReadOnlyOrderedMap<'TRowKey,'TValue>) (append:IReadOnlyOrderedMap<'TRowKey,'TValue>) : bool =
...
//        this.AddLast(i.Key, i.Value)
  ...
  interface IUpdateable<'TRowKey,'TValue> with
    [<CLIEvent>]
...
//  new(dictionary:IDictionary<'TRowKey,'TValue>) = IndexedMap(Some(dictionary), Some(dictionary.Count), Some(Comparer<'TRowKey>.Default :> IComparer<'TRowKey>))
//  new(capacity:int) = IndexedMap(None, Some(capacity), Some(Comparer<'TRowKey>.Default :> IComparer<'TRowKey>))
//
//  // do not expose ctors with comparer to public
//  internal new(comparer:IComparer<'TRowKey>) = IndexedMap(None, None, Some(comparer))
//  internal new(dictionary:IDictionary<'TRowKey,'TValue>,comparer:IComparer<'TRowKey>) = IndexedMap(Some(dictionary), Some(dictionary.Count), Some(comparer))
//  internal new(capacity:int,comparer:IComparer<'TRowKey>) = IndexedMap(None, Some(capacity), Some(comparer))
//
//  internal new(comparer:IEqualityComparer<'TRowKey>) = 
//    let comparer' = 
//      {new IComparer<'TRowKey> with
//          member this.Compare(x,y) = 
//            if comparer.Equals(x,y) then 0 else -1
//      }
//    IndexedMap(None, None, Some(comparer'))
//
//  internal new(dictionary:IDictionary<'TRowKey,'TValue>,comparer:IEqualityComparer<'TRowKey>) = 
//    let comparer' = 
//      {new IComparer<'TRowKey> with
//          member this.Compare(x,y) = 
//            if comparer.Equals(x,y) then 0 else -1
...
//
//  internal new(capacity:int,comparer:IEqualityComparer<'TRowKey>) =
//    let comparer' = 
//      {new IComparer<'TRowKey> with
//          member this.Compare(x,y) = 
//            if comparer.Equals(x,y) then 0 else -1
...
//
//
//  static member internal OfSortedKeysAndValues(keys:'TRowKey[], values:'TValue[], size:int) =
...
//    let im = new IndexedMap<'TRowKey,'TValue>()
//    im.keys <- keys
//    im.size <- size
//    im.values <- values
//    im
//
//  static member OfSortedKeysAndValues(keys:'TRowKey[], values:'TValue[]) =
