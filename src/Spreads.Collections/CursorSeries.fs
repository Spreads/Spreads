...
type CursorBinding<'K,'V,'V2 when 'K : comparison>(cursorFactory:unit->ICursor<'K,'V>) =
  ...
  interface IEnumerator<KVP<'K,'V2>> with    
    ...
    member x.MoveNext(): bool =
      if hasValidState then
        ...
        while not found && x.InputCursor.MoveNext() do // NB! x.InputCursor.MoveNext() && not found // was stupid serious bug
...
type RepeatCursor<'K,'V  when 'K : comparison>(cursorFactory:unit->ICursor<'K,'V>) as this =
  inherit CursorBinding<'K,'V,'V>(cursorFactory)
  do
...
type MapValuesCursor<'K,'V,'V2 when 'K : comparison>(cursorFactory:unit->ICursor<'K,'V>, mapF:'V -> 'V2) =
  inherit CursorBinding<'K,'V,'V2>(cursorFactory)
...
type AddIntCursor<'K when 'K : comparison>(cursorFactory:unit->ICursor<'K,int>, addition:int) =
  inherit CursorBinding<'K,int,int>(cursorFactory)
...
type AddInt64Cursor<'K when 'K : comparison>(cursorFactory:unit->ICursor<'K,int64>, addition:int64) =
  inherit CursorBinding<'K,int64,int64>(cursorFactory)
...
type LogCursor<'K when 'K : comparison>(cursorFactory:unit->ICursor<'K,int64>) =
  inherit CursorBinding<'K,int64,double>(cursorFactory)
...
type SeriesExtensions () =
    ...
    [<Extension>]
    static member inline Map(source: Series<'K,'V>, mapFunc:'V->'V2) : Series<'K,'V2> = 
      CursorSeries(fun _ -> new MapValuesCursor<'K,'V,'V2>(source.GetCursor, mapFunc) :> ICursor<'K,'V2>) :> Series<'K,'V2>
