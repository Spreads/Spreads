...
//  let inline ( <*) a b = lift2 (fun z _ -> z) a b
  ...
  type TaskBoolBuilder(?continuationOptions, ?scheduler, ?cancellationToken) =
      let contOptions = defaultArg continuationOptions TaskContinuationOptions.None
      let scheduler = defaultArg scheduler TaskScheduler.Default
      let cancellationToken = defaultArg cancellationToken CancellationToken.None

      member this.Return x = returnMBool x

      member this.Zero() = returnM()

      member this.ReturnFrom (a: Task<bool>) = a

      member this.Bind(m, f) = bindBool f m // bindWithOptions cancellationToken contOptions scheduler f m
      member this.BindUnit(m, f) = bind f m
      ...
      member this.While(guard, m) =
          let rec whileRec(guard, m) = 
            if not(guard()) then this.Zero() else
                this.BindUnit(m(), fun () -> whileRec(guard, m))
          whileRec(guard, m)

      member this.While(guardTask:unit->Task<bool>, body) =
        let m = guardTask()
        let onCompleted() =
          this.BindUnit(body(), fun () -> this.While(guardTask, body))
        if m.Status = TaskStatus.RanToCompletion then 
          onCompleted()
        else
          let tcs =  new TaskCompletionSource<_>() // (Runtime.CompilerServices.AsyncTaskMethodBuilder<_>.Create())
  ...
  let taskBool = TaskBoolBuilder(scheduler = TaskScheduler.Current)