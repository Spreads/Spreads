...
namespace Spreads.Core.Tests.Cursors
{
    ...
    public class MapValuesTests
    {
        ...
        public void CouldMapValues()
        {
            ...
            var map2 = new MapValuesSeries<int, int, int, ICursor<int, int>>(map.After(0), i => i * 2);
            var map3 = new MapValuesSeries<int, int, int, ICursor<int, int>>(map.After(2), i => i * 2);
        ...
        public void CouldMapValuesViaExtensionMethods()
        {
            ...
            var map2 = map.After(0).Map(i => i * 2).Map(i => i * 2);
            Assert.AreEqual(2, map.First.Value);
        ...
        public void CouldMapValuesViaExtensionMethodsBenchmark()
        {
            ...
            for (int r = 0; r < 10; r++)
            {
                var sw = new Stopwatch();
                sw.Restart();
                var map = sm.Map(i => i * 2).Map(i => i * 2).Map(i => i * 2);
                long sum = 0;
                foreach (var kvp in map)
                {
                    sum += kvp.Value;
