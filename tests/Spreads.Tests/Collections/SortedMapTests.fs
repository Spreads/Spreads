...
type SortedMapTests() =
    ...
    member this.``Nested sorted list - optimized``() =
        for c in 0..19 do
            ...
            let lim = 1000000L
            let bucket_size = 256
            let hc = Int64HashComparer(512us)
    ...
    member this.``SHM - Could insert and read``() =
        ...
        for round in 0..(rounds-1) do
            ...
            let nsl = SortedHashMap<int64, float>(Int64HashComparer(512us)) //TimePeriodHashComparer())
...
//            watch.Stop()
    ...
    member this.``SHM - Could insert concurrently``() =
        ...
        for round in 0..(rounds-1) do
            let lim = 1000000L
            let s = ref (new SortedHashMap<int64, int64>(Int64HashComparer(512us)))
