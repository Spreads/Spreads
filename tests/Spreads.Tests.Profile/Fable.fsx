...
////////////////////////////////////////////////////////////////////////////////////////////
        ...
        /// FastGenericComparer
        let fgc<'K when 'K : comparison> = Comparer<'K>.Default
...
//    open SpreadsCollections
    ...
    type ImmutableSortedMap<[<EqualityConditionalOn>]'K,[<EqualityConditionalOn;ComparisonConditionalOn>]'V when 'K : comparison >
      ...
      member this.Keys with get() = (this :> IEnumerable<KeyValuePair<'K,'V>>) |> Seq.map (fun kvp -> kvp.Key)
      [<ObsoleteAttribute("Naive impl, optimize if used often")>]
      member this.Values with get() = (this :> IEnumerable<KeyValuePair<'K,'V>>) |> Seq.map (fun kvp -> kvp.Value)
