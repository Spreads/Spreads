...
// NB https://stackoverflow.com/questions/6629891/module-values-in-f-dont-get-initialized-why
// Must compile as library, but the project setting has no effect in tests so turn value in a function
let idxValues () =  { 
...
let ``Merging series sub-ranges works as expected`` () =
  ...
  let ranges = 
    ((merged.Index :?> Spreads.Deedle.Indices.Virtual.VirtualOrderedIndex<DateTimeOffset>).Source 
